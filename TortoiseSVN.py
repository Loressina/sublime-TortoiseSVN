...
class TortoiseSvnCommand(sublime_plugin.WindowCommand):
    def run(self, cmd, paths=None, isHung=False):
        ...
        proce = subprocess.Popen(
            cmd.encode(pathEncoding) if pathEncoding else cmd, stdout=subprocess.PIPE
        )
...
class SvnUpdateCommand(MutatingTortoiseSvnCommand):
    def run(self, paths=None):
        settings = self.get_setting()
        closeonend = "3" if settings.get("autoCloseUpdateDialog") else "0"
        MutatingTortoiseSvnCommand.run(self, f"update /closeonend:{closeonend}", paths)


class SvnCommitCommand(TortoiseSvnCommand):
    def run(self, paths=None):
        settings = self.get_setting()
        closeonend = "3" if settings.get("autoCloseCommitDialog") else "0"
        TortoiseSvnCommand.run(self, f"commit /closeonend:{closeonend}", paths)
...
class SvnBlameCommand(TortoiseSvnCommand):
    def run(self, paths=None):
        ...
        TortoiseSvnCommand.run(self, f"blame /line:{str(row)}", paths)
